...
func WithContentFieldName(name string) *contentFieldNamer {
	c := contentFieldNamer(name)
	return &c
}

func (c *contentFieldNamer) AddToEncoder(e *Encoder) *Encoder {
	e.contentFieldName = string((*c))
	return e
}

func (c *contentFieldNamer) AddToDecoder(d *Decoder) *Decoder {
	d.contentFieldName = string((*c))
	return d
}
